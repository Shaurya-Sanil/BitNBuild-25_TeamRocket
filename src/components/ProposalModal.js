...
import { proposalService } from '../firebase/database';

const ProposalModal = ({ project, isOpen, onClose, onSubmit, user }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await proposalService.createProposal(proposal, project.id, user.uid);
      onSubmit(proposal);
      onClose();
    } catch (error) {
