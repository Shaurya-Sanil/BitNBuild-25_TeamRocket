...
const Header = ({ user, onLogout }) => {
  ...
  { path: '/project/process', icon: CheckCircle, label: 'Progress' },
  ...
  const clientNav = [
    ...
    { path: '/client/post-project', icon: Plus, label: 'Post Project' },
    { path: '/client/my-projects', icon: Briefcase, label: 'My Projects' },
    { path: '/client/browse-freelancers', icon: Users, label: 'Browse Freelancers' },
    { path: '/client/view-bids', icon: MessageCircle, label: 'Bids' }
  ];
        ...
        <div className="flex items-center justify-between" style={{ height: '100%' }}>
          ...
          >
            {navItems.map((item) => {
              ...
              // active if exact match or the current path starts with the item's path (covers nested routes)
            ...
            {user ? (
              <>
                ...
                <div style={{ position: 'relative' }}>
                  ...
                  >
                    <Avatar src={user?.avatar} alt={user?.displayName || 'User'} size={32} />
                ...
                {/* Menu (hamburger) removed per request */}
              </>
            ) : (
              <Link to="/auth" className="button button-primary" style={{ textDecoration: 'none' }}>Log in</Link>
            )}
          </div>
        </div>
      </div>
